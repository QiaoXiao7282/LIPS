...
def parse_args():
    ...
    parser.add_argument('--beta', type=int, default=100, help='Used to control the collaboration of critical parameters')

    ## model
    parser.add_argument('--model', type=str, default='ResNet6', help='model name')
    ...
    parser.add_argument('--client_num', type=int, default=100, help='The number of clients')
    parser.add_argument('--local_eps_first', type=int, default=5, help='The number of epoch for each round')
    parser.add_argument('--local_eps_first_d', type=int, default=1.0, help='The practical density ratio for masking: can be updated')

    ## dst
    parser.add_argument('--dst', type=str, default='rigl', help='default: rigl (with sparsity)') ## rigl, local
    parser.add_argument('--tau', type=float, default=0.0, help='The ratio of local critical parameters (default=0.0, without using local weights)')
    parser.add_argument('--density_init', type=float, default=1.0, help='default: 1.0 (dense initialization)')
    parser.add_argument('--prune_method', type=str, default='magnitude', help='pruning method on client')  ## sensity, magnitude, random
    parser.add_argument('--regrow_method', type=str, default='topk', help='regrow method on server')  ## topk, lowk, randomk

    parser.add_argument('--prune_ll', type=str, default='large', help='pruning from large to small')
    parser.add_argument('--update_mode', type=str, default='global_sel', help='how to make use of server model')  ## global_sel, global_sel_init, local, fdst

    parser.add_argument('--no_margin', type=str2bool, default=True, help='defaul: True (without include first and last layer)')
...
exp_args = dict(
    ...
    other=dict(test_freq=1, logging_path=f'./logging/cifar10_fedcac_resnet/{args.noniid}_{args.alpha}_{args.tau}_{args.beta}_{args.dst}_{args.density_init}_{args.seed}')
)
